...
const getType = (type) => {
	const types = [ 'string', 'number', 'boolean', 'integer', 'array' ]
	return types.includes(type) ? type : 'string'
}
/**
...
function createPostPushPutOutput(verbs, definitions, pathKeys, defaultResponse) {
	...
	pathParams = _.filter(pathParams, function(param) {
		...
		return !(['query', 'path'].includes(param.in) && !hasKey)
	});
...
function createVerbDefinitions(verbs, definitions) {
	...
	// 	fieldArrayResult = createFieldArrayDefinitions(verbs.success.fields["Success 200"], definitions, defaultObjectName, defaultObjectName);		
...
function createFieldArrayDefinitions(fieldArray, definitions, topLevelRef, defaultObjectName) {
	...
	for (var i = 0; i < fieldArray.length; i++) {
		...
		definitions[objectName] = definitions[objectName] ||
			{  title: objectName, type: "object", properties : {}, required : [] };

		if (nestedName.propertyName) {
			var prop = { type: getType((parameter.type || "").toLowerCase()), description: removeTags(parameter.description) };
			...
			if(typeIndex !== -1 && typeIndex === (type.length - 2)) {
				prop.type = "array";
				prop.items = {
					type: getType(type.slice(0, type.length-2).toLowerCase())
				};
			}
...
function createNestedDefinitions(type, definitions, field, topLevelRef) {
	...
	if (schema) {
		if (definitions[topLevelRef]) {
			...
			if (definitions[topLevelRef]['required'].indexOf(field) === -1) {
				definitions[topLevelRef]['required'].push(field);
			}
		} else {
			definitions[topLevelRef] = { title: topLevelRef, type: "object", properties: { [field]: schema }, required : [field]};
		}
...
function createParameters(parameters) {
	...
	for (var i = 0; i < parameters.length; i++) {
		...
		parsedParams.push({
			name: param.field,
			in: param.group === "url" ? "path" : param.group === "file" ? "formData" : param.group.toLowerCase(),
			required: !param.optional,
			type: getType(param.type.toLowerCase()),
			description: removeTags(param.description)
		});

		if (param.type.indexOf('[]') !== -1) {
			parsedParams[i].type = 'array'
			parsedParams[i].items = { 
				type: getType(param.type.substr(0, param.type.length - 2).toLowerCase())
			}
		}
	}
